...
startCLI()                         core/chat.js
  ...
  └─ prompt()  ◄────────────────────────────────────────┐ loop
   ...
   runTurn(input, session)                               │
         ...
         ├─ if first turn:
         │     getProjectContext()       context.js       (auto-pick relevant files)
         │     webSearch()               search.js        (Tavily, optional)
         ...
         │         onChunk(text) ──► sanitize (hide edit blocks) ──► ☻ prose,
         │                          or keep spinner while code generates
         │         collect tool_calls
...
`startCLI()` calls `prompt()`, which uses `readline`'s `rl.question("❯ ")`. The
...
Every non-empty line is saved to `~/.noble/history` (`saveHistoryEntry`).
...
3. **On the first turn only**, appends:
   - `--- PROJECT CONTEXT ---` from `getProjectContext` (a ranked slice of the
     repo, see [context](#context-what-the-model-sees)).
   - `--- WEB SEARCH ---` from `webSearch` (Tavily; empty if no API key).
   These are *not* re-sent on later turns — the model is expected to use tools to
   fetch anything else it needs.
4. Pushes the assembled `{ role: "user", content }` onto `session.messages` and
   increments `session.turnCount`.

> **Why first-turn-only context?** It seeds the model cheaply without re-paying
> the token cost every turn. Follow-ups rely on `read_file` / `grep` / `list_dir`.
...
- **`session`** (`newSession()`): `{ messages, turnCount, lastResponse,
  lastUserInput, allowlist, formatReminder }`. The `messages` array is the full
  chat history sent to Ollama every turn. `formatReminder` is a one-shot flag that
  makes the next turn nag the model to use a writable file format. Saved/loaded as
  JSON via `/save` / `/load`.

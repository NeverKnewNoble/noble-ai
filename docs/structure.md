...
│   ├── context.js        Auto-context: full ranked files + lightweight file tree
...
│   ├── search.js         Tavily web search (optional, first substantive turn)
│   ├── ui.js             Brand palette + colorizers (the one place to reskin)
│   └── header.js         The startup banner: blue ASCII logo + status panel
├── docs/                 ← you are here
...
| File | Purpose | Key exports |
| --- | --- | --- |
| `core/context.js` | Walks the repo and scores files against the prompt's keywords (priority boosts for README/manifests) to pack a char-budgeted context block, **and** produces a lightweight file-tree-only snapshot for project questions. | `getProjectContext`, `getProjectTree` |
...
| `core/search.js` | Optional Tavily web search, appended to the first substantive turn. No-op without `TAVILY_API_KEY`. | `webSearch` |
...
| `core/header.js` | Clears the screen and prints the startup banner: a solid-blue ANSI-shadow "NOBLE AI" logo, tagline + version, and a rounded status panel (model, RAM, git branch, project, path). | `renderHeader` |

### Cross-cutting

| File | Purpose | Key exports |
| --- | --- | --- |
| `core/ui.js` | The brand layer: the blue palette (`c`), an optional gradient colorizer, and helpers. Change this to reskin the whole UI. | `c`, `BRAND`, `gradient`, `sampleGradient` |
